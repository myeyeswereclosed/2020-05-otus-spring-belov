package ru.otus.spring.spring_shell_testing_app.parser;
...
import ru.otus.spring.spring_shell_testing_app.service.parser.CsvTestFileParser;
...
public class CsvTestFileParserTest {
    ...
    public void successfulParsing() {
        var parser = new CsvTestFileParser(readerMock(CORRECT_TEST_FILE_CONTENTS), configMock());
    ...
    public void parsingFailure() {
        INCORRECT_TEST_FILE_CONTENTS.forEach(
            contents -> {
                var parser = new CsvTestFileParser(readerMock(contents), configMock());
    ...
    private Config configMock() {
        Mockito.when(config.getQuestionsDelimiter()).thenReturn(",");
        Mockito.when(config.getQuestionItemsMinNumber()).thenReturn(5);
